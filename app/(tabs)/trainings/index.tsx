...
const Trainings = () => {
  ...
  const {
    ...
    isRefetching: isRefetchingProgram,
  } = usePrograms();
  const {
    quizzes,
  ...
  const handleQuizClick = async () => {
    ...
    await onQuizStart(selectedQuiz?.id);
      ...
      <View className="flex-1 px-6">
        {category === "Programas" && (
          <ScrollableList
            ...
            refreshControl={
              <RefreshControl
                refreshing={isRefetchingProgram}
        ...
        {category === "Quizzes" && (
          <View>
            <FlatList
              ...
              className={cn(noRunningQuiz.length > 0 && "h-[200px]")}
